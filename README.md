...
```bash
# Run MCP server tests and generate diagrams
python3 test_mcp.py

# Generate fresh documentation diagrams
python3 generate_documentation_diagrams.py
```

**Output Files:**
The test script generates diagrams as part of the MCP tool testing process. Generated diagrams include:
- SVG with embedded icons (self-contained, scalable)
- Full resource metadata and configuration details

**Viewing Generated Diagrams:**
Check the `examples/diagrams/` directory for SVG files generated by `generate_documentation_diagrams.py`.
...
- Interactive HTML with clickable resources
- SVG output with embedded icons for scalable vector graphics
...
1. **sample-plan.json** — 6 resource changes showcasing basic AWS infrastructure
...
**Generate diagrams from examples:**
```bash
python3 generate_documentation_diagrams.py
```
...
```bash
# Test MCP tool integration and generate example diagrams
python3 test_mcp.py
```
